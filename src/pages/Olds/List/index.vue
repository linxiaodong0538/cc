<template>
  <div>
    <NavBar :title="consts.ALIASES[where.alias] + ' - 老人列表'"></NavBar>
    <div class="main">
      ...
      <Padding v-if="items && items.length" :dirs="['top']">
        <van-cell-group>
          <van-cell v-for="item in items" :key="item.id" :title="item.name" is-link
                    @click="$router.push(`/olds/${where.alias}/details/${item.id}`)" />
        </van-cell-group>
        <Padding>
          <van-pagination
            v-model="pagination.current"
            :page-count="Math.ceil(olds.olds.total / consts.PAGE_SIZE)"
            mode="simple" />
        </Padding>
      </Padding>
    </div>
